...
class SheetJob(InstanceScoped):
    ...
    KIND_ITEM, KIND_TEST, KIND_IMAGE = "item", "test", "image"
    kind = models.CharField(max_length=8, default="item")   # #178 test records per row; #179 image rows
    ...
    def counts(self) -> dict:
        ...
        c = {"create": 0, "patch": 0, "test": 0, "image": 0, "skip": 0, "error": 0, "pending": 0, "done": 0,
             "created": 0, "updated": 0, "posted": 0, "attached": 0, "failed": 0}
        for r in self.rows:
            ...
            if r.get("state") == "pending":
                ...
                c["done"] += 1
                d = r.get("done") or []
                c["created" if "created" in d else "posted" if "test posted" in d
                  else "attached" if "attached" in d else "updated"] += 1
