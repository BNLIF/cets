...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not os.path.isdir(femb_ocr_dir):
            raise CommandError(f"FEMB_OCR_DIR '{femb_ocr_dir}' does not exist or is not a directory.")
        ...
        for filepath in filepaths:
            try:
                ...
                for line in lines:
                    ...
                    if "FEMB" in raw_type:
                        ...
                        if femb_serial_number != sn:
                            self.stdout.write(self.style.WARNING(f"  - Serial number in filename ({femb_serial_number}) does not match serial number in file ({sn}). Skipping file."))
                            continue
                    else:
                        comp_type = ""
                ...
                if not femb_version:
                    self.stdout.write(self.style.WARNING(f"  - Could not find FEMB version in {filename}. Skipping file."))
                    continue

                # Check if FEMB already exists
                if not FEMB.objects.filter(serial_number=femb_serial_number, version=femb_version).exists():
                ...
                self.stdout.write(self.style.ERROR(f"  - Error processing file {filepath}: {e}"))
        ...
        if new_fembs_to_create:
            ...
            for femb_data in new_fembs_to_create:
                self.stdout.write(f"  - Serial Number: {femb_data['serial_number']}, Version: {femb_data['version']}")
                    ...
                    for comp_data in components_to_update:
                        ...
                        if model:
                            ...
                            if obj.status == 'on-femb' and not created:
                                self.stdout.write(self.style.WARNING(f"  - Skipping {comp_data['type']} {comp_data['serial_number']} as it is already on a FEMB."))
                                continue
                            
                            femb = FEMB.objects.get(serial_number=comp_data['femb_serial_number'], version=comp_data['femb_version'])
                            obj.status = 'on-femb'
                            obj.femb = femb
