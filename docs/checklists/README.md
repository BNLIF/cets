...
| `assembly` | — | `{position: PID}` — the item's positions with what sits in them, read live on the fill page (Hajime 2026-09-20: the Item View's assembly list inside a checklist, since QC testers link sub-components as they attach them), each row with an *Unlink* button, and a *Link items →* link to the item's packing page for placing, moving and swapping; the submit stores the list as it stands after the checklist's own links. Nothing to fill in; on a blank/offline form a hint takes its place |
