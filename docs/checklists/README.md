...
| `table` | `columns` (required), `units`, tolerance as above | `{column: number}` — a column may be `{"label": …, "formula": "C1 + C2*(C3/C4)"}` (#109): computed from the row's other cells (`C<n>` = 1-based column), read-only in the form, recomputed server-side at submit; in the editor write it as `Total = C1 + C2`. A formula may reference computed columns to its left; unresolvable = the cell stays empty. A dict column may also carry its own `nominal`/`tol` or `min`/`max`, replacing the table-wide range for that cell — editor syntax `Total = C1 + C2 [11 ± 1]` or `P1 [1.4..1.6]`. `{"label": …, "color": "yellow"}` tints that column's cells (#139, e.g. the one column the user fills in a parts list; header excluded, beats a row tint) — editor syntax `Batch No. {yellow}`, or the picker in each header cell of the *rows…* grid. `{"label": …, "check": true}` makes a pass/fail column (#116) — a compact tri-state per cell, stored as `true`/`false`, blank omitted; editor syntax `OK = check`. `{"label": …, "text": "3.1+-0.1"}` makes a fixed non-editable cell (#114, the HVS "Expected" column) — editor syntax `Expected = "3.1+-0.1"` (no commas in the text); the constant is stored with the submission, formulas can reference it when it's a plain number or a `value ± tolerance` string like `3 +- 1` (the formula reads the value, the record keeps the text), and a table where nothing was typed is still omitted. `"rows": [{"label": "Length", "texts": {"Expected": "3.1"}, "color": "yellow"}, …]` (#119) makes a multi-row table: one row per entry with a label cell on the left, the columns shared; a row's `texts`, `ranges` (per-row accepted ranges, `{column: {nominal, tol}}` or `{min, max}`) and `color` override the table's/column's; formulas and ranges apply per row (`C<n>` stays within the row). The value becomes `{row: {column: value}}`; untouched rows are omitted. No `rows` = the single implicit row, stored and rendered exactly as before. Editor: the *rows…* button opens a grid — header cells are the column syntax, body cells a row's fixed text (blank = an input), a tint per row. A table-level `"color"` (#115) tints the cells (not the header) to match the reference drawing — `yellow`/`green`/`blue`/`red`/`gray` (light shades) or `#rrggbb`, painted as picked while the inputs keep their own ground; prefer light colors. Anything else is dropped. The editor offers a color picker with those swatches predefined. Cells have borders like a real table by default (#139); `"grid": false` turns them off — the editor's *borders* checkbox. `"link": true` (#153, Anselmo's SiPM boards) makes a **linking table**: every plain cell (not check / fixed / computed) takes a PID, with a Scan and a Pick button under it (Hajime; Pick needs the type) — with `type_id` and `sn` as on `qr` a serial number is resolved to the PID of the item of that type carrying it, anything else is dropped; a table without a `type_id` takes the one type the target's positions accept (all of a supercell's positions take SiPM boards), several types leave it PIDs-only — and the items are linked as sub-components: a *link now* button under the table does it at once (one PATCH, first free position accepting each item's type, items already linked left alone, linked cells paint green), and the submit does the same. *check* beside it is the dry run: nothing written, each filled cell gets the position it would take or what stops it — not a PID, a duplicate, no free position, unknown to HWDB, a status HWDB won't link — as its tooltip, the status line summing up. `"into": "<position>"` links them into the item sitting in that position of THIS item instead — the PDS module's supercell in `SC1` holding 8 boards — so one table per supercell; an empty position is an error naming it. **Phone scanning** (#170, Anselmo: a row is one physical location): the 📱 beside a row label, or *scan with phone* under the table, opens a panel with the QR / URL of the scan page (the packing page's, opened for this item, keeping serial numbers as scanned, and carrying the table's type and serial pattern — the phone refuses a PID of another type, a string off the pattern, and a code it already sent, before anything is sent, and with the type known the server refuses a serial number no item of the type carries, or one that several carry, naming them); the fill page polls the item's scan feed and drops each code into the next empty cell of that row (the table: reading order, wrapping) as if typed — type guard, serial lookup and *check* apply unchanged — with a dashed cursor on the cell that takes the next scan (click a cell to move it), a duplicate already in the table refused, and the listener stopping when the row is full or the ✕ is pressed. The page tells the phone what became of every scan — "→ Left · 2", "already in the table", "row “Left” is full — not placed", "nobody was listening — not placed" (a scan sent before the row's button was pressed) — shown on the phone's line for it; nothing heard in 15 s = "no checklist page is listening". One row or table listens at a time; a packing page open in another tab never sees these scans (the feed is scoped to the item). Editor: tick *link the cells as sub-components*, then Type ID / serial pattern / *into* |
| `text` / `textarea` | — | string |
| `datetime` | — | `YYYY-MM-DDTHH:MM` string |
| `select` | `options` (required) | chosen option |
| `qr` | `type_id` (optional, #112), `sn` (optional, #149) | scanned/typed string — with `type_id` set (e.g. `D00300100002`), a code of any other type won't register: the scanner keeps scanning, typed input paints red, and a mismatch is dropped at submit. With `sn` also set — a regex such as `HPK\d{5}` — a typed, barcoded or Scan-Text value matching it in full is taken as a serial number (a SiPM strip ID is stored as the item's serial number) and resolved to the PID of the item of that type carrying it (#149: the mirror first, then one HWDB lookup), swapped in with `HPK19877 → Z00100300029-05316` noted beside the box; a serial no item of the type has paints red and is dropped at submit like a mismatch. A serial that SEVERAL items carry (#168 — HWDB doesn't enforce unique serials; 38 shared ones on the SiPM type) is never resolved: the box keeps the serial, paints red and names the items ("serial number X is on 2 items: PID, PID — enter the PID"), and a submit with one still typed is refused with the same message. The Plots page's paste list and the embedded plot field take ALL of them (plotting every holder shows which one to keep — Chao) and name the serial with its PIDs. Without `sn` every value that isn't a PID is looked up as a serial number; `sn` needs a `type_id` to scope the lookup. On an item's page a `qr` / `link` field also gets a **phone** button (#171, Anselmo: *Scan* uses the camera of the device showing the page): the same panel as a linking table's — the scan page's QR / URL for this item with the field's type and pattern — and one scan from the phone fills the field as if typed (type guard, serial lookup, live link button), the phone hearing "→ ‹label›" or the refusal. A typed field also gets a **Pick** button (#134): a dialog lists the type's mirrored items — the Shipping Procedure's four linkable statuses by default, *show all statuses* adds the rest display-only — with a search box and pages of 200; clicking a row fills the field |
...
| `imagemap` | `image_id` (required), `slots` (required: `[{label, x, y, type_id?}]`, percent coords; a slot's own `type_id` guards that slot over the field's — Hajime 2026-09-25, the CRU's 12 adapter boards of 7 types), `type_id` (optional — guards every slot without its own), `linked` (with `link`: `"open"` default, `"collapsed"`, `"off"` — the linked-positions list beside the map, Hajime 2026-09-25), `sn` (optional, #149 — as `qr`), `link` (optional) | `{slot label: PID}` (#113, Top CRP) — the drawing renders with a numbered dot per slot; tapping a dot scans into that slot (green once filled), and a compact list below takes typed input. Values go to the test record; with `"link": true` (#133, editor checkbox) every scanned item is also linked into the item's subcomponents at submit, in one PATCH — a slot named like one of the type's functional positions goes there, any other slot takes the first free position for the scanned item's type; items already linked are left alone, an occupied named position fails by slot label. On the fill page such a map also shows the item's positions live (occupant or empty, like box packing; an empty slot named like a position is pre-filled with its occupant) and a *link* button beside every scanned slot that patches that item in right away — no submit needed — turning into *unlink* once it sits in a position. Every slot gets a Pick button (#134): with a `type_id` on the map it lists that type; on a linking map without one, a slot named like a position lists that position's type and any other slot lists every type the free positions accept (a type selector and a Type column appear when there are several) — free items only, with a count of those hidden because they already sit inside another item. Linking (live button or submit) and unlinking stamp the new parent in the mirror at once, so a just-linked item leaves the lists. In the editor: pick the image, then click it to drop slots — or edit the `label @ x, y` lines directly |
| `assembly` | `into` (optional) | `{position: PID}` — the item's positions with what sits in them (with `"into": "<position>"` — Chao 2026-09-21 — the positions of the sub-assembly sitting in that position of THIS item, e.g. the supercell in `SC1`: the heading names it, *Link items →* opens ITS page, *Unlink* patches it, the submit snapshots its list, and an empty position is an error naming it; editor: the *the item in position…* box), read live on the fill page, each occupant's PID a link to its item page (Hajime 2026-09-20: the Item View's assembly list inside a checklist, since QC testers link sub-components as they attach them), each row with an *Unlink* button, and a *Link items →* link to the item's packing page for placing, moving and swapping; the submit stores the list as it stands after the checklist's own links. Nothing to fill in; on a blank/offline form a hint takes its place |
| `plot` | `plot` (required — a Plots page URL, the type + `#` state), `sections` (optional — section titles to read from; blank = every section), `fields` (optional — field labels to read from, e.g. one table of three in a section; with `sections`, within those; a section or field name the form doesn't have is named under the empty box — "no section named “SiPM boards” in this form" — never guessed: Anselmo 2026-09-21), `sn` (optional — regex a value must match in full to count as a serial number), `reference` (optional, `true` — the page exactly as pasted, with its own PID range or filter, nothing read from the form: a distribution to compare a fresh measurement with; Hajime) | nothing — display only (#160). The Plots page is embedded with the PIDs and serial numbers of the URL's type found in the form: values of `qr` / `link` / `imagemap` slots and plain table cells alike, PIDs of the type as PIDs, any other single word as a serial candidate (`sn` narrows that). Entries match an item by PID or by serial number — case-insensitive, leading zeros in a digit run ignored (`HPK19843` finds `HPK019843`); the status line reports how many matched and lists the rest. The plot follows the form as it is filled (about a second after a change), *Open in Plots* opens the full page with the same items as an editable PID filter — on THIS server: only the type and the `#` state are kept from the pasted URL, so a link copied on twister works on a local test server and vice versa. Needs the network; the sub-component type's test data must be synced on the Plots page once. Recipe (PDS module, Anselmo): build the Vbd plot on the SiPM board type's Plots page for one board, *Copy link*, paste it as the field's URL, name the sections that hold the board IDs. Tolerance: the Series pane's *Limits* boxes (low / high) draw a dotted line at each value — across a value plot, upright on a histogram — with the axis stretched to show them; the select beside them reads the boxes as absolutes or as offsets below / above the highlighted series' mean, an amount or a percentage (Anselmo: a SiPM tolerance is relative and vendor-dependent); they travel in the URL |
...
(SC1, SC2, SC3, SC4)") with a checkbox each, a status select (default
Waiting on QA/QC Tests) and QA/QC uploaded / certified boxes; the "virtual
part" button presets Passed All + both flags (Hajime's rule for bureaucratic
