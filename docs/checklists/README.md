...
| `imagemap` | `image_id` (required), `slots` (required: `[{label, x, y}]`, percent coords), `type_id` (optional — guards every slot), `link` (optional) | `{slot label: PID}` (#113, Top CRP) — the drawing renders with a numbered dot per slot; tapping a dot scans into that slot (green once filled), and a compact list below takes typed input. Values go to the test record; with `"link": true` (#133, editor checkbox) every scanned board is also linked into the item's subcomponents at submit, in one PATCH — a slot named like one of the type's functional positions goes there, any other slot takes the first free position for the board's type; boards already linked are left alone, an occupied named position fails by slot label. On the fill page such a map also shows the item's positions live (occupant or empty, like box packing) and a *link* button beside every scanned slot that patches that board in right away — no submit needed — turning into *unlink* once it sits in a position. In the editor: pick the image, then click it to drop slots — or edit the `label @ x, y` lines directly |
