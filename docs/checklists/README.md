...
| `qr` | `type_id` (optional, #112) | scanned/typed string — with `type_id` set (e.g. `D00300100002`), a code of any other type won't register: the scanner keeps scanning, typed input paints red, and a mismatch is dropped at submit. A typed field also gets a **Pick** button (#134): a dialog lists the type's mirrored items — the Shipping Procedure's four linkable statuses by default, *show all statuses* adds the rest display-only — with a search box and pages of 200; clicking a row fills the field |
