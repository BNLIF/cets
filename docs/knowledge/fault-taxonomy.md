...
| `high_noise` | channel | Channel RMS exceeds 2.5× median of all 128 channels | Check cable shielding and ground connections; verify ADC bias settings via `wib_cfgs`; recurring single-channel excess after re-test → suspect the LArASIC serving that channel. Before concluding, check the raw waveform (`tools/femb_wave.py`) — a `baseline_jump` masquerades as high RMS |
| `baseline_jump` | channel | Baseline steps between discrete levels (tens–hundreds of ADC counts, dwell times of hundreds of samples) while pulse response and calibration pass; inflates RMS across many configs, often worst at high gain or with leakage current enabled. Only visible in the raw waveform — the RMS summary plots show just the elevated RMS | Confirm with a waveform plot (`tools/femb_wave.py --scan <ch>` then `--ch`); a common known issue. Re-run once; if it recurs, suspect the LArASIC channel serving it (input/reset side) and log the affected channel in the board notes |
